...
from simple_arm.srv import GoToPosition, GoToPositionResponse
import numpy as np
...
def clamp_at_boundaries(requested_j1, requested_j2):
	...
	if not min_j1 <= requested_j1 <= max_j1:
		clamped_j1 = np.clip(requested_j1, min_j1, max_j1)
	...
	if not min_j2 <= requested_j2 <= max_j2:
		clamped_j2 = np.clip(requested_j2, min_j2, max_j2)
