...
from simple_arm.srv import GoToPosition, GoToPositionResponse, GoToPositionRequest
...
def coord_equal(coord_1, coord_2):
...
class LookAway(object):
    ...
    def joint_states_callback(self, data):
        if coord_equal(data.position, self.last_position):
            self.arm_moving = False
        else:
    ...
    def look_away_callback(self, data):
        if not self.arm_moving and uniform_image(data.data):
            try:
